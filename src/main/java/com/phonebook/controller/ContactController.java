    ...
    public String saveOrUpdateContact(@ModelAttribute("command") Contact c, Model m, HttpSession session) {
        ...
        if (contactId == null) {
            try {
                ...
                c.setUserId(userId);//Foreign key of the logged in user's ID
                contactService.save(c);
                return "redirect:clist?act=sv";//redirect user to the Contact List URL
            } catch (Exception e) {
                e.printStackTrace();
            ...
            try {
                c.setContactId(contactId); //Primary key
